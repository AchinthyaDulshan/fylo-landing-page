    ...
    return (
        <div className='bg-Desaturated-Blue py-16 px-6 md:px-5'>
            <div className='max-w-7xl mx-auto grid lg:grid-cols-2  text-Light-Grayish-Blue items-center text-center lg:text-left'>
                ...
                <div>
                    <form action="#" method="get" className='grid gap-4 pt-6 lg:pt-0' noValidate onSubmit={(e) => handleSubmit(e, () => console.log('Form submitted ' + email))}>
                        <div>
                            <input type="email" name="" id="" placeholder='email@example.com' className={`border rounded-md text-Desaturated-Blue shadow-md px-4 py-2 lg:w-3/5 w-4/5 mx-auto lg:mx-0 ${error && ('border-2 border-red-600')}`} onChange={(e) => setEmail(e.target.value)} value={email} required />
                            {error && (
